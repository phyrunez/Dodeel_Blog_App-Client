    ...
    useEffect(() => {
        if(!isLoading) {
            setBreadCrumbsData([
                { name: 'Home', link: '/' },
                { name: 'Blog', link: '/blog' },
                { name: `${data.title}`, link: `https://digitalonboarderscds.com.ng/blog/${data.slug}` }
            ])
        } 
                        ...
                        <div className="mx-auto max-w-5xl mt-10 text-sm leading-7">
                            ...
                            <div className="text-[#333333] text-center text-xs">
                                <div className="mt-8 mb-3 flex flex-row gap-x-2">
                                    ...
                                    <img className="cursor-pointer h-4 w-4 mb-1" src={images.Polygon} onClick={() => setProfileDropdown(!profileDropdown)} alt="youtube" /><span className={`${profileDropdown ? 'block' : 'hidden' }`}><SocialShareButtons url={`https://digitalonboarderscds.com.ng/blog/${data.slug}`} title={data.title} /></span>
